  ...
  methods: {
    handleError (index) {
      ...
      switch (index) {
        case 0:
          {
            if (!/^[\u4e00-\u9fa5|\w]{6,20}$/.test(item.value)) { this.textlist[index].errmsg = '帐号为6-20位汉字，数字或字母组合！' }
          }
          break
        case 1:
          {
            if (!/^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/.test(item.value)) { this.textlist[index].errmsg = 'email格式不正确！' }
          }
          break
        case 2:
          {
            if (!/^\w{6,20}$/.test(item.value)) { this.textlist[index].errmsg = '密码为6-20位数字或字母组合！' }
          }
          break
        case 3:
          {
            if (item.value != this.textlist[2].value) { this.textlist[index].errmsg = '两次密码不一致！' }
          }
          break
      }
    },
    signUp (oauthUId, oauthToken, type) {
      ...
      $this.$http.jsonp(Config.apiUrl + Config.urlsEnum.register,
        ...
        if (res.result == 1) {
          ...
          if (res.result == 0) { $this.msg.content = res.msg }
        }
      })
    },
    test () {
      if (Common.checkTextEdit(this.textlist)) {
        var msg = ''
        if (!/^[\u4e00-\u9fa5|\w]{6,20}$/.test(this.textlist[0].value)) { msg += '帐号为6-20位汉字，数字或字母组合！' } else if (!/^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/.test(this.textlist[1].value)) { msg += 'email格式不正确！' } else if (!/^\w{6,20}$/.test(this.textlist[2].value)) { msg += '密码为6-20位数字或字母组合！' } else if (this.textlist[2].value != this.textlist[3].value) { msg += '两次密码不一致！' }
        if (msg.length > 0) {
