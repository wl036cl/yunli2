  ...
  methods: {
    ...
    getSports () {
      if (this.userId && this.token) {
        ...
        let $this = this
        this.$http
          .jsonp(Config.apiUrl + Config.urlsEnum.getSports, {
            params: {
              uid: $this.userId,
              token: $this.token,
              npage: 1
    ...
    setData (data) {
      if (data && data.length > 0) {
        for (let i = 0; i < data.length; i++) {
          ...
          data[i].isPrivate = data[i].isPrivate == 1
        }
      }
      this.list = data
      if (this.list) {
    ...
    deleteSprotJsonP (index) {
      let sId = index > -1 ? this.list[index].sportId : 0
      this.$http
        .jsonp(Config.apiUrl + Config.urlsEnum.deleteSport, {
          params: {
            uid: this.userId,
            token: this.token,
            sid: sId
...
var touchMove = {
  ...
  bindEvent: function () {
    ...
    Common.addEvent(this.el, 'touchstart mousedown', function (e) {
      if (
        !$this.disabled &&
        ($this._isPointerType = $this.isPointerEventType(e, 'down')) &&
    ...
    Common.addEvent(this.el, 'touchmove mousemove', function (e) {
      if (!$this.disabled && $this.index > -1) {
        ...
        if (
          ($this._isPointerType = $this.isPointerEventType(e, 'move')) &&
          !$this.isPrimaryTouch(e)
        ) { return }
        ...
        if (
          ($this.touch.x1 != 0 || $this.touch.y1 != 0) &&
          ($this.touch.x2 != 0 || $this.touch.y2 != 0)
        ) {
    ...
    Common.addEvent(this.el, 'touchend mouseup', function (e) {
      if (!$this.disabled && $this.index > -1) {
        if (
          ($this._isPointerType = $this.isPointerEventType(e, 'up')) &&
          !$this.isPrimaryTouch(e)
        ) { return }

        if ($this.ishorizontally == 0 || $this.touch.x2 == 0) {
          // 垂直运行的||没有move
          return
        }
        ...
        if (
          $this.delta < 320 &&
          Math.abs($this.touch.x2 - $this.touch.x1) > 10
        ) {
          var direction = $this.swipeDirection(
            $this.touch.x1,
            $this.touch.x2,
  ...
  init: function (_el, list) {
    ...
    this.el = _el
    this.list = list
    this.win_width = _el.clientWidth * 0.96
    this.bindEvent()
  }
}
</script>
