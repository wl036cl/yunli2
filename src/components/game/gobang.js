...
GoBang.prototype = {
  ...
  undo: function () { // 悔棋
    if (this.history.length > 0) {
      ...
      if (this.history[len - 1][0] == this.playerType || this.playerType == 3) { this.removeChess([this.history[len - 1]]) } else if (len > 1) { this.removeChess([this.history[len - 1], this.history[len - 2]]) }
    }
  },
...
var ComputerAI = {
  ...
  judge: function (x, y, chessType) {
    ...
    // 优先该回合的落子成五(黑子走，并能成5)
    if (a > 100000 && chessType === 1) { a = a * 2 } else if (b > 100000 && chessType === 2) { b = b * 2 }
    var result = a + b
