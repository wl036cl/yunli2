  ...
  data () {
    return {
      ...
      chart: {
        ...
        canvasId: 'myCanvas',
        type: 'line',
        width: Math.round(document.body.clientWidth * 0.96 * Math.round(window.devicePixelRatio)),
        height: Math.round(document.body.clientWidth * 0.96 * Math.round(window.devicePixelRatio) / 1.25),
        data: [],
        options: {
          title: '近7次',
      ...
      if (list && list.length) {
        ...
        if (this.chart.category === 0) {
          for (let i = 0, len = list.length; i < len; i++) {
            let m = list[i]
            if (i == 0 || i == len - 1) { title += Common.dateFtt('yyyy.M.d', new Date(m.recordTimeStr)) + (i == 0 ? '-' : '') }
          ...
          for (let i = 0, len = list.length; i < len; i++) {
            let m = list[i]
            if (i == len - 1) { title += Common.dateFtt('yyyy.M.d', new Date(m.recordTimeStr)) }
