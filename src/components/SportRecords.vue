...
import Schart from './vue-schart/vue-schart.vue'
...
export default {
  data () {
    return {
      ...
      chart: {
        ...
        type: 'line',
        width: Math.round(
          document.body.clientWidth * 0.96 * Math.round(window.devicePixelRatio)
        ),
        height: Math.round(
  ...
  methods: {
    ...
    addRecord () {
      this.$emit('routerContent', 'sportrecordedit', {
        sid: this.sid,
        sindex: this.sindex
      })
    },
    deleteRecordJsonP (index, rid) {
      this.$http
        .jsonp(Config.apiUrl + Config.urlsEnum.deleteSportRecord, {
          params: {
            uid: this.userId,
            token: this.token,
            rid: rid
        ...
        .then(function (res) {
          ...
          if (res.result == 1) {
            Common.setCache([
              ['yunli_records_' + this.sid],
              ['yunli_sport_' + this.sid],
              ['yunli_sports_' + this.userId]
            ])
    ...
    getRecords () {
      if (this.userId && this.token && this.sid) {
        let list = Common.getCache('yunli_records_' + this.sid),
          model = Common.getCache('yunli_sport_' + this.sid)
        ...
        this.$http
          .jsonp(Config.apiUrl + Config.urlsEnum.getSportRecords, {
            params: {
              uid: this.userId,
              token: this.token,
              sid: this.sid,
      ...
      if (list && list.length) {
        ...
        if (this.chart.category === 0) {
          for (let i = 0, len = list.length; i < len; i++) {
            ...
            if (i == 0 || i == len - 1) {
              title +=
                Common.dateFtt('yyyy.M.d', new Date(m.recordTimeStr)) +
                (i == 0 ? '-' : '')
            }
          ...
          // 近8周
          let computeNumType = 'sum'
          let lastGroupValue = 0
          if (this.model.unit.toLocaleLowerCase() === 'kg') {
            computeNumType = 'average'
          }

          function toPushData (date, count, arr) {
            let computedValue =
              arr && arr.length ? arrValue.reduce((a, b) => a + b) : 0
            if (computeNumType === 'average') {
              if (computedValue != 0) {
                computedValue = Number((computedValue / arr.length).toFixed(2))
              } else {
                computedValue = lastGroupValue
              }
            }
            lastGroupValue = computedValue
            return {
              name: Common.dateFtt('M.d', date) + '~' + '(' + count + ')',
              value: computedValue
            }
          }
          ...
          for (let i = 0, len = list.length; i < len; i++) {
            ...
            let _sunday = new Date(
              today.getTime() - today.getDay() * 24 * 3600 * 1000
            )
            sunday = sunday || _sunday
            if (
              Common.dateFtt('yyyy-MM-dd', sunday) !=
              Common.dateFtt('yyyy-MM-dd', _sunday)
            ) {
              // 新的一周（周日）
              _list.push(toPushData(sunday, count, arrValue))
              let nextSunday = new Date(
                sunday.getTime() + 7 * 24 * 3600 * 1000
              )
              while (
                Common.dateFtt('yyyy-MM-dd', _sunday) >
                Common.dateFtt('yyyy-MM-dd', nextSunday)
              ) {
                // 中途有周没有数据
                _list.push(toPushData(nextSunday, 0, []))
                sunday = nextSunday
            ...
            arrValue.push(m.recordNum)
            // let computedValue = arrValue.reduce((a, b) => a + b)
            count++
            if (i == len - 1) {
              _list.push(toPushData(sunday, count, arrValue))
            }
          }
          if (computeNumType === 'sum') {
            title += '--总数'
          } else if (computeNumType === 'average') {
            title += '--波动'
          }
          ...
          function toPushData (date, count, arr) {
            ...
            lastGroupValue = computedValue
            return {
              name: Common.dateFtt('M', date) + '月' + '(' + count + ')',
              value: computedValue
            }
          }
          ...
          for (let i = 0, len = list.length; i < len; i++) {
            ...
            if (
              Common.dateFtt('yyyy-MM-dd', firstday) !=
              Common.dateFtt('yyyy-MM-dd', _firstday)
            ) {
              // 新的一周（周日）
              _list.push(toPushData(firstday, count, arrValue))
              let year = Number(Common.dateFtt('yyyy', firstday)),
                month = Number(Common.dateFtt('M', firstday))
              month += 1
              ...
              while (
                Common.dateFtt('yyyy-MM-dd', _firstday) >
                Common.dateFtt('yyyy-MM-dd', nextFirstday)
              ) {
                // 中途有月没有数据
                _list.push(toPushData(nextFirstday, 0, 0))
                firstday = nextFirstday;
                (year = Number(Common.dateFtt('yyyy', firstday))),
                (month = Number(Common.dateFtt('M', firstday)))
                month += 1
                year = month > 12 ? year + 1 : year
                month = month > 12 ? month - 12 : month
    ...
    getChart (type) {
      ...
      if (this.chart.width === 0) {
        this.chart.width = (this.$el.clientWidth * 0.96).toFixed(0)
      ...
      if (this.list && this.list.length) {
        if (type === 0) {
          ...
          this.setChart(
            this.list.length > 10 ? this.list.slice(0, 10) : this.list
          )
        } else if (type === 1) {
          // 近8周
          this.chart.type = 'bar'
          ...
          let firstSunday = new Date(
            new Date(
              Common.dateFtt('yyyy-M-d', new Date(this.list[0].recordTimeStr))
            ).getTime() -
              (7 * 7 + new Date(this.list[0].recordTimeStr).getDay()) *
                24 *
                3600 *
                1000
          )
          ...
          // if (this.list.length > 10) {
          let date = Common.dateFtt(
            'yyyy-M',
            new Date(this.list[0].recordTimeStr)
          )
...
.sport-record .timeline,
.sport-record .chart-menu,
.sport-record .chart-canvas {
  width: 96%;
  margin: 1rem auto;
}
.sport-record .mu-tabs {
...
.sport-record .mu-icon,
.sport-record .mu-tab-text {
