...
<script>
    ...
    export default {
        ...
        data () {
            return {
                textlist:[
                    {  maxlength: 6, placeholder: '完成量', type: 'tel', value: '',errmsg:'',list:null },
                    {  maxlength: 200, placeholder: '完成时间', type: 'date', value:Common.dateFtt('yyyy-MM-dd',new Date()),max:Common.dateFtt('yyyy-MM-dd',new Date()), errmsg: '', list: null },
                    {  maxlength: 200, placeholder: '心得体会', type: 'multiline',rows:2,rowsMax:5, value: '', errmsg: '', list: null }
                ],
                msg: {
                    isRight: true,
                    content: ''
        ...
        methods: {
            ...
            editRecord() {
                if(this.userId&&this.token&&this.sid) {
                    this.disabled=true;
                    this.$http.jsonp(Config.apiUrl + Config.urlsEnum.editSportRecord,
                            {
                                params: {
                                    uid: this.userId, token: this.token,
                                    sid: this.sid,
