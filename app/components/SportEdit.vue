        ...
        methods: {
            handleError(index){
                ...
                switch (index) {
                    case 0:
                    {
                        if (!/^[\u4e00-\u9fa5|\w]{1,10}$/.test(item.value))
                            this.textlist[index].errmsg = "项目名称为10个字以内！";
                    }
                        break;
                    case 1:
                    {
                        if (!/^[\d|\.]{1,6}$/.test(item.value))
                            this.textlist[index].errmsg = "项目目标为6位内的正数！";
                    }
                        break;
                }
            },
            ...
            test(){
                if (Common.checkTextEdit(this.textlist)) {
                    var msg = "";
                    if (!this.textlist[0].value.length)
                        msg += '名称不能为空';
                    else if (!Number(this.textlist[1].value))
                        msg += '目标不能为空';
                    else if (!this.textlist[2].value.length)
                        msg += '计量单位不能为空';
                    else if (!this.textlist[3].value.length)
                        msg += '运动宣言不能为空';
