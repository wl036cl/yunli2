...
GoBang.prototype = {
  ...
  undo: function () {
    ...
    if (this.history.length > 0) {
      ...
      if (this.history[len - 1][0] == this.playerType || this.playerType == 3) this.removeChess([this.history[len - 1]]);
      else if (len > 1) this.removeChess([this.history[len - 1], this.history[len - 2]]);
    }
  },
  ...
  addChess: function (x, y, chessType) {
    //落子
  ...
  computerPlay: function (chessType) {
    ...
    if (position) this.addChess(position[0], position[1], chessType);
    //检查输赢
    var result = this.checkWin();
    this.getResult(result);
    return result;
  ...
  startMenu: function () {
    this.drawMask();
    //设置菜单按钮
    var width = this.lineHeight * 5,
      height = this.lineHeight,
      padding = (this.height - (2 * 3 - 1) * this.lineHeight) / 2;
  ...
  getResult: function (winner) {
    //结局
    if (winner) {
      this.drawMask();
      this.status = 0; //结束
      //设置按钮
      var width = this.lineHeight * 3,
        height = this.lineHeight,
        padding = (this.height - (2 * 3 - 1) * this.lineHeight) / 2;
      this.btnList = [];
  ...
  drawButton: function (pixelX, pixelY, width, height, txt, eventName, options) {
    ...
    if (options && options.hasOwnProperty('textAlign')) this.ctx.textAlign = options.textAlign;
    if (options && options.hasOwnProperty('color')) this.ctx.fillStyle = options.color;
    if (options && options.hasOwnProperty('font')) this.ctx.fillStyle = options.font + 'px Arial';
  ...
  drawChessBoard: function () {
    ...
    for (var i = 0; i < 15; i++) {
      this.ctx.moveTo(this.padding + 0.5 + i * this.lineHeight, this.positionY + this.padding - 0.5);
      this.ctx.lineTo(this.padding + 0.5 + i * this.lineHeight, this.positionY + this.chessHeight - this.padding - 0.5);
      this.ctx.moveTo(this.padding + 0.5, this.positionY + this.padding + i * this.lineHeight - 0.5);
      this.ctx.lineTo(this.chessWidth - this.padding + 0.5, this.positionY + this.padding + i * this.lineHeight - 0.5);
    }
    this.ctx.stroke();
    //边框
    this.ctx.beginPath();
    ...
    this.ctx.moveTo(0.5, this.positionY + 0.5);
    this.ctx.lineTo(this.chessWidth - 0.5, this.positionY + 0.5);
    this.ctx.lineTo(this.chessWidth - 0.5, this.positionY + this.chessHeight - 0.5);
    this.ctx.lineTo(0.5, this.positionY + this.chessHeight - 0.5);
    this.ctx.closePath();
    this.ctx.stroke();
    ...
    function drawPoint(obj, x, y) {
      ...
      obj.ctx.arc(obj.padding + 0.5 + x * obj.lineHeight, obj.padding + 0.5 + y * obj.lineHeight + obj.positionY, obj.chessRadius * 0.4, 0, Math.PI * 2, true);
      obj.ctx.fill();
    }
    drawPoint(this, 3, 3), drawPoint(this, 12, 3), drawPoint(this, 3, 12), drawPoint(this, 12, 12);
  },
  drawChess: function (x, y, chessType, isSign) {
    //绘制棋子
    var pixelX = this.padding + 0.5 + x * this.lineHeight;
    var pixelY = this.padding + 0.5 + y * this.lineHeight + this.positionY;
    this.ctx.beginPath();
...
var ComputerAI = {
    ...
    for (var x = 0; x < 15; x++) {
      for (var y = 0; y < 15; y++) {
        if (this.chess[x][y] === 0) {
          ...
          if (_score > score || (x === 7 && y === 7 && _score == score)) {
            //持黑先走优先落中间,
            score = _score;
            a[0] = x;
            a[1] = y;
  ...
  judge: function (x, y, chessType) {
    ...
    //优先该回合的落子成五(黑子走，并能成5)
    if (a > 100000 && chessType === 1) a = a * 2;
    else if (b > 100000 && chessType === 2) b = b * 2;
    var result = a + b;
